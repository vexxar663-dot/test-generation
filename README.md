# Equation Dodge — генератор этажей

Прототип алгоритма генерации комнат из раздела 3 ГДД «Equation Dodge».
Python-версия нужна, чтобы проверить алгоритм и баланс **до** того, как писать
код под Godot: здесь за секунду прогоняются тысячи этажей и видно, что
получается.

📄 **[Алгоритм и замеры → docs/room_generation.md](docs/room_generation.md)**
🎨 **[Школьная планировка и реальные спрайты → docs/sprites.md](docs/sprites.md)**

![Этаж на спрайтах](docs/images/pixels_floor.png)

Тот же этаж схемой — так удобнее читать структуру:

![Схема этажа](docs/images/floor.png)

## Запуск

```bash
python3 -m roomgen show --seed 12     # этаж в терминале, без зависимостей
python3 -m roomgen check --count 2000 # прогон проверок шага 8 на 2000 сидах
python3 -m roomgen render --out out   # PNG: схема, лист сидов, статистика
python3 -m roomgen pixels --seed 12   # PNG: этаж реальными спрайтами проекта
python3 -m roomgen export --seed 12   # JSON с сеткой тайлов для сцены Godot
python3 -m unittest discover -s tests # 28 тестов
```

Генератор, тайловая сетка и проверки работают на голом stdlib. matplotlib нужен
для `render`, Pillow — для `pixels` (`pip install -r requirements.txt`).

## Что внутри

| Файл | Что делает |
|---|---|
| `roomgen/config.py` | все числа из ГДД в одном месте: лимиты типов, вероятности, тайминги |
| `roomgen/generator.py` | шаги 1–7: якоря, основной путь, тупики, заполнение, развилки |
| `roomgen/validation.py` | шаг 8: связность, лимиты, «лестница только за боссом» |
| `roomgen/ascii_render.py` | этаж в терминале |
| `roomgen/viz.py` | схема, лист сидов и графики статистики на matplotlib |
| `roomgen/tilemap.py` | комнаты → школьное здание в тайлах (кабинеты, коридоры, проёмы) + экспорт в JSON |
| `roomgen/sprites.py` | атлас реальных спрайтов из `spites/` |
| `roomgen/render_sprites.py` | рендер этажа пиксель-артом |
| `spites/` | спрайты проекта, как в Godot (`res://spites/`) |
| `tests/` | инварианты, детерминированность, распределения, тайловая сетка |

## Результат на 1500 сидах

- 18.5 комнат на этаже в среднем (ГДД: 16–22, в среднем 18–20)
- до босса обязательно пройти 5.25 комнаты, 87% этажей — 5 или 6 (ГДД: 5–6)
- сундук и усложнённая комната — на 100% этажей, ивент — на 51% (ГДД: 50%)
- кабинет 20×15 тайлов ≈ 8.2×6.2 м — настоящий школьный класс
- 0 этажей с нарушениями, в среднем 1.01 попытки генерации
- один сид всегда даёт один и тот же этаж

## Три расхождения с ГДД

1. **«Основной путь 5–6 комнат» невозможен цепочкой комнат** — от (0,0) до (4,4)
   без диагоналей минимум 9. Зато школьная планировка выполняет требование
   буквально: мимо комнат можно пройти коридором, и обязательных остаётся 5–6.
2. **Порядок шагов ГДД ломает ивентовые комнаты** — в порядке 4→5→6→7 ивент
   выпадает на 1% этажей вместо 50%. Заполнение перенесено перед конвертациями.
3. **Двери нельзя выводить из соседства клеток** — иначе заполнение этажа
   превращает тупик с сундуком в проходной коридор. Двери хранятся явным
   списком рёбер.

Подробности и замеры — в [docs/room_generation.md](docs/room_generation.md).
