...
### 3.1. «Основной путь 5–6 комнат» / «6–10 шагов» невыполним

Манхэттенское расстояние от (0,0) до (4,4) равно 8, ходить можно только по
горизонтали и вертикали, значит **короче 9 комнат путь спавн → босс не бывает**.
Чётность разрешает только 9, 11, 13. Собственный пример из ГДД
(`(0,0) → (0,1) → (1,1) → (2,1) → (2,2) → (3,2) → (4,2) → (4,3) → (4,4)`) —
ровно 9 комнат, то есть пример противоречит тексту рядом с ним.

Взято: 9 комнат (55%), 11 (35%), 13 (10%) — `main_path_lengths`.
Если 5–6 комнат до босса нужны по геймплею, менять надо не алгоритм, а
постановку: сетка 4×4, либо босс не в углу, либо часть пути за одной дверью.
...
| Комнат до босса | «5–6» (невыполнимо) | 9 / 11 / 13 |
...
1. **Время этажа.** По таблице таймингов ГДД зачистка на 100% выходит
   **2.9–7.1 мин**, середина оценки — 4.8 мин при заявленных 5–7. Этаж
   ...
   либо на этаж нужно ещё 2–3 комнаты.
...
2. `FloorData` (`Resource`) — результат генерации, живёт в `Global.gd`.
3. `Minimap.gd` — рисует ту же карту, что и `viz.py`, по тем же глифам и цветам.
4. `Room.tscn` на `TileMapLayer`: двери включаются по четырём булям
   (север/юг/запад/восток) из списка дверей.
5. Порт тестов в GUT или простой отладочный прогон 1000 сидов при старте.
